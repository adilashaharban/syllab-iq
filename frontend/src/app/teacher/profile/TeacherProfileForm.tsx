...
import { Loader2, User, Phone, School, Briefcase, Save } from "lucide-react";
...
export function TeacherProfileForm({ initialData }: TeacherProfileFormProps) {
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      {/* Department */}
      <div className="space-y-1.5">
        <label htmlFor="department" className="text-sm font-medium text-foreground/80 flex items-center gap-2">
      ...
      {/* Employee ID */}
      <div className="space-y-1.5">
        <label htmlFor="employeeId" className="text-sm font-medium text-foreground/80 flex items-center gap-2">
          <Briefcase className="h-4 w-4 text-indigo-400" />
          Employee ID
        </label>
        <input
